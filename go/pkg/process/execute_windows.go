...
func setupProcessAttributes(cmd *exec.Cmd) {
	// This isolates children for termination but preserves parent's signal handling
	cmd.SysProcAttr = &syscall.SysProcAttr{
		CreationFlags: syscall.CREATE_NEW_PROCESS_GROUP,
		// CREATE_NEW_PROCESS_GROUP: Enables our termination logic (sendCtrlBreak)
		// No DETACHED_PROCESS: Preserves parent's console signal handling
	}

	// Note: This approach:
	// 1. Child processes in separate group (enables graceful termination)
	// 2. Parent retains normal console signal handling in cmd.exe
