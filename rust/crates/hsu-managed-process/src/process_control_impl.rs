...
use hsu_process_management::config::{ProcessConfig, HealthCheckConfig, ResourceLimitsConfig};
use hsu_process_management::lifecycle::ProcessLifecycleManager;
...
impl ProcessControlImpl {
    ...
    pub fn new(
        ...
        let lifecycle_manager = ProcessLifecycleManager::new(
            config.management.restart.strategy.clone().into(),
            config.management.restart.max_attempts,
        );
    ...
    async fn spawn_process(&mut self) -> ProcessResult<()> {
        ...
        // Spawn with timeout
        let spawn_timeout = self.config.management.control.startup_timeout;
        match timeout(spawn_timeout, cmd.spawn()).await {
            Ok(Ok(child)) => {
            ...
            Ok(Err(e)) => {
                ...
                Err(CommonProcessError::Execution {
                    id: self.config.id.clone(),
                    command: self.config.management.control.executable.clone(),
                    reason: err.details,
                })
            }
            Err(_) => {
                let err = ProcessError::new(
                    ...
                    format!("Process spawn timed out after {:?}", spawn_timeout),
                    true,
                );
                self.last_error = Some(err.clone());
    ...
    async fn terminate_process(&mut self) -> ProcessResult<()> {
        ...
        if let Some(ref mut child) = self.child {
            ...
            if let Err(e) = timeout(shutdown_timeout, child.wait()).await {
                ...
                match check_http_health(&endpoint, timeout_duration).await {
                    Ok(data) => {
                        ...
                        if data.is_healthy {
                            ...
                            status.record_failure(data.error_message.unwrap_or_else(|| "Unknown failure".to_string()));
                    ...
                    Err(e) => {
                        let mut status = health_status.write().await;
                        status.record_failure(format!("Health check error: {}", e));
                ...
                match resource_monitor.get_process_resource_usage(pid) {
                    Ok(usage) => {
                        ...
                        if let Some(ref limits_config) = limits {
                            let violations = check_resource_limits(&usage, limits_config);
...
impl ProcessControl for ProcessControlImpl {
    async fn start(&mut self) -> ProcessResult<()> {
        ...
        self.state_machine.transition_to_starting()
            .map_err(|e| CommonProcessError::StateTransition {
                id: self.config.id.clone(),
                from: self.state_machine.current_state(),
    ...
    async fn restart(&mut self, force: bool) -> ProcessResult<()> {
        ...
        self.lifecycle_manager.reset_counters();
    ...
    fn get_diagnostics(&self) -> ProcessDiagnostics {
        ...
        ProcessDiagnostics {
            ...
            failure_count: self.lifecycle_manager.consecutive_failures(),
